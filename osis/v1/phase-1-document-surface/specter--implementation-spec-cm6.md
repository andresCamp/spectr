# Specter Phase 1 — Implementation Spec (CM6 Decoration)
...
```text
.md file on disk
  -> SpecterDocument.text (markdown string, sole source of truth)
...
- Update `SpecterDocument.readableContentTypes` to `[.markdown]`
...
- File association: `.md` files open in Specter from Finder
...
**Verify:** A developer can use Specter as their daily spec viewer. Open files, read in rendered mode, make quick edits, save, switch to raw mode for tricky syntax.
...
- `SpecterDocument { text: String }` — the markdown string, unchanged
