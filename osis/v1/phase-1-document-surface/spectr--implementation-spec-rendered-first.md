# Spectr Phase 1 — Implementation Spec (Rendered-First)
...
`SpectrDocument` remains the persisted `FileDocument` type:
...
- `SpectrDocument.text` for persistence
...
Use a markdown parser to build the structured model from `SpectrDocument.text`.

Recommended path:

- use Apple's `swift-markdown` package or another AST-capable parser
- map parser output into Spectr's own `MarkdownDocumentModel`
...
That pattern is valid, but it is not the product target. Spectr's rendered mode should stay visually clean and syntax-free, with raw mode reserved for literal markdown editing.
...
The node/range mapping is essential. It lets Spectr answer:
...
To work correctly with `FileDocument`, `SpectrDocument.text` should stay reasonably synchronized in memory during editing.
...
For Spectr, the difference is that the backing store is a real `.md` file on disk, not an internal notes database.
...
This is the cleanest architecture if Spectr's real goal is:
