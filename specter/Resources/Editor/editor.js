(() => {
  ...
  function inlineDOMAtPos(parent2, pos) {
    let dom = parent2.dom, { children } = parent2, i = 0;
  ...
  function posAtCoords(view2, coords, precise, bias = -1) {
    ...
    if (element && ((_a2 = view2.docView.nearest(element)) === null || _a2 === void 0 ? void 0 : _a2.isEditable) != false) {
      ...
      if (node && (!view2.contentDOM.contains(node) || browser.safari && isSuspiciousSafariCaretResult(node, offset, x) || browser.chrome && isSuspiciousChromeCaretResult(node, offset, x)))
        node = void 0;
      if (node)
  ...
  function isSuspiciousChromeCaretResult(node, offset, x) {
    if (offset != 0)
      return isEndOfLineBefore(node, offset, x);
    for (let cur = node; ; ) {
      let parent2 = cur.parentNode;
      if (!parent2 || parent2.nodeType != 1 || parent2.firstChild != cur)
        return false;
  ...
  var DOMReader = class {
    ...
    readRange(start, end) {
      ...
      for (let cur = start; ; ) {
        ...
        if ((view2 && nextView ? view2.breakAfter : (view2 ? view2.breakAfter : isBlockElement(cur)) || isBlockElement(next) && (cur.nodeName != "BR" || cur.cmIgnore) && this.text.length > oldLen) && !isEmptyToEnd(next, end))
          this.lineBreak();
        cur = next;
      }
  ...
  function isEmptyToEnd(node, end) {
    ...
    for (; ; node = node.nextSibling) {
      if (node == end || !node)
        break;
      let view2 = ContentView.get(node);
      if (!((view2 === null || view2 === void 0 ? void 0 : view2.isWidget) || node.cmIgnore))
        return false;
      if (view2)
  ...
  function openTags(doc2, tree) {
    ...
    for (let parent2 = findParentElement(tree); parent2 && !parent2.type.isTop; parent2 = findParentElement(parent2.parent)) {
      let tagName = elementName2(doc2, parent2);
  ...
  function completeTag(state, schema, tree, from, to) {
    ...
    return {
      from,
      to,
      options: allowedChildren(state.doc, parent2, schema).map((tagName) => ({ label: tagName, type: "type" })).concat(openTags(state.doc, tree).map((tag, i) => ({
        label: "/" + tag,
        apply: "/" + tag + end,
        type: "type",
        boost: 99 - i
    ...
    return StateField.define({
      ...
      provide(field) {
        return [
          ...
          EditorView.atomicRanges.of((view2) => view2.state.field(field).atomicRanges)
        ];
      }
    });
  }
    ...
    syntaxTree(state).iterate({
      enter(node) {
        switch (node.name) {
          ...
          case "ATXHeading5":
          case "ATXHeading6":
            decorateHeading(decorations2, atomicRanges2, node, state);
            break;
          case "StrongEmphasis":
            decorateStrongEmphasis(decorations2, atomicRanges2, node, state);
            break;
          case "Emphasis":
            decorateEmphasis(decorations2, atomicRanges2, node, state);
            break;
          case "InlineCode":
            decorateInlineCode(decorations2, atomicRanges2, node, state);
            break;
          case "Link":
  ...
  function addReplacement(decorations2, atomicRanges2, from, to, spec = {}) {
    ...
    decorations2.push(Decoration.replace(spec).range(from, to));
    atomicRanges2.push(createAtomicRange(from, to));
  }
  function addMark(decorations2, from, to, className, attributes) {
    if (from >= to) {
      return;
    }
    decorations2.push(
  ...
  function addLineClass(decorations2, from, className) {
    decorations2.push(Decoration.line({ class: className }).range(from));
  }
  function decorateHeading(decorations2, atomicRanges2, node, state) {
    ...
    addReplacement(decorations2, atomicRanges2, line.from, prefixEnd);
  ...
  function decorateEmphasis(decorations2, atomicRanges2, node, state) {
    const text = state.sliceDoc(node.from, node.to);
    const delimiter = text[0];
    if (!delimiter || delimiter !== text[text.length - 1] || !["*", "_"].includes(delimiter)) {
      return;
    }
    addReplacement(decorations2, atomicRanges2, node.from, node.from + 1);
    addMark(decorations2, node.from + 1, node.to - 1, "cm-emphasis");
    addReplacement(decorations2, atomicRanges2, node.to - 1, node.to);
  }
  function decorateInlineCode(decorations2, atomicRanges2, node, state) {
    const text = state.sliceDoc(node.from, node.to);
  ...
  function decorateLink(decorations2, atomicRanges2, node, state) {
    const text = state.sliceDoc(node.from, node.to);
  ...
  function decorateBlockquote(decorations2, atomicRanges2, node, state) {
    ...
    for (let lineNumber = startLine; lineNumber <= endLine; lineNumber += 1) {
      ...
      addLineClass(decorations2, line.from, "cm-blockquote-line");
    }
  }
  function decorateFencedCode(decorations2, atomicRanges2, node, state) {
  ...
  function toggleMarkdownWrap(delimiter) {
    return (view2) => {
      ...
      const afterTo = Math.min(view2.state.doc.length, to + length);
      const before = view2.state.sliceDoc(beforeFrom, from);
      const after = view2.state.sliceDoc(to, afterTo);
  ...
  function backspaceHeadingMarker(view2) {
  ...
  function indentListSelection(view2) {
  ...
  function outdentListSelection(view2) {
    const lines = selectedLines(view2.state);
    const changes = [];
    let touched = false;
    for (const line of lines) {
  ...
  function setTheme(theme2) {
    document.documentElement.dataset.theme = theme2 === "dark" ? "dark" : "light";
    const light = document.getElementById("theme-light");
    const dark = document.getElementById("theme-dark");
    if (light instanceof HTMLLinkElement) {
      light.disabled = theme2 === "dark";
    }
